...
private:
	...
	iterator_template(_Node* __node, const hashtab<_Value,_Key,_HashFunc,_Allocator> * __this):_tab(__this),_np(0),_node(__node)
	{
		if(__node)
		{
			int n = _tab->get_hash(__node->_key);
...
public:
	...
	void swap(hashtab& __rhs) 
	{ 
		abase::swap(_hash,__rhs._hash);
		abase::swap(_num_elements,__rhs._num_elements);
		_buckets.swap(__rhs._buckets);
	}
	...
	size_type max_bucket_count() const {return __abase_prime_list[(int)__stl_num_primes - 1]; } 
