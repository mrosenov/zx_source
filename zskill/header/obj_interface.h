...
namespace TOPIC_SITE { struct topic_data; }
...
public:
	...
	void Heal(const XID & healer,unsigned int life, bool can_heal_blood_pool);
	void Heal(unsigned int life, bool is_potion, bool can_heal_bloodpool);
...
public:
	...
	void ClearRandomSpecFilter(int mask,int count);
...
public:
	...
	void SendRegionEnchant1Msg(const A3DVECTOR& pos, float radius,enchant_msg & enchant, unsigned int max_count, std::vector<exclude_target>& target_exclude);
	void Enchant(const XID & target, enchant_msg & msg);
	void EnchantZombie(const XID & target, enchant_msg & msg);

	void RegionEnchant1(A3DVECTOR const& pos, float radius, enchant_msg& msg, unsigned int mc, std::vector<exclude_target, std::allocator<exclude_target> >& target_exclude);
	void RegionEnchant2(A3DVECTOR const& pos, float radius, enchant_msg& msg, unsigned int mc, std::vector<exclude_target, std::allocator<exclude_target> >& target_exclude);
	void RegionEnchant3(A3DVECTOR const& pos, float cos_half_angle, enchant_msg& msg, unsigned int mc, std::vector<exclude_target, std::allocator<exclude_target> >& target_exclude);
	
	// void RegionEnchant1(const A3DVECTOR& pos, float radius,enchant_msg & msg,int mc, std::vector<exclude_target>& target_exclude);			//球
	// void RegionEnchant2(const A3DVECTOR& pos, float radius,enchant_msg & msg,int mc, std::vector<exclude_target>& target_exclude);			//柱
	// void RegionEnchant3(const A3DVECTOR& pos, float cos_half_angle,enchant_msg & msg,int mc, std::vector<exclude_target>& target_exclude);	//椎
	...
	bool IsBindState();
	bool IsQilinLeader(const XID & target);
	...
	void SendClientAchieveData(unsigned int size, const void * data);
	void SendClientAchieveFinish(unsigned short achieve_id, int achieve_point, int finish_time);
	void SendClientPremissData(unsigned short achieve_id, char premiss_id,  unsigned int size, const void * data);
...
public:
	...
	void BeTaunted(const XID & who,int time);
	void BeTaunted2(const XID & who,int time);
...
public:
	...
	void EnhanceResistance(unsigned int cls, int res);	 //cls = [0,5]
	void ImpairResistance(unsigned int cls, int res);

	void EnhanceScaleResistance(unsigned int cls, int res);	 //cls = [0,5] res 千分制
	void ImpairScaleResistance(unsigned int cls, int res);

	void EnhanceResistanceTenaciy(unsigned int cls, int res);	 //cls = [0,5]
	void ImpairResistanceTenaciy(unsigned int cls, int res);

	void EnhanceScaleResistanceTenaciy(unsigned int cls, int res);	 //cls = [0,5] res 千分制
	void ImpairScaleResistanceTenaciy(unsigned int cls, int res);

	void EnhanceResistanceProficiency(unsigned int cls, int res);	 //cls = [0,5]
	void ImpairResistanceProficiency(unsigned int cls, int res);

	void EnhanceScaleResistanceProficiency(unsigned int cls, int res);	 //cls = [0,5] res 千分制
	void ImpairScaleResistanceProficiency(unsigned int cls, int res);
...
public:
	...
	void AddMoney(unsigned int inc);
	...
	int GetMafiaTrashBoxDetail(GDB::itemdata * list, unsigned int size);
	...
	unsigned int GetMafiaTrashBoxCapacity();
	...
	unsigned int GetPetBedgeInventorySize();
	int GetPetBedgeInventoryDetail(GDB::itemdata * list, unsigned int size);
	unsigned int GetPetEquipInventorySize();
	int GetPetEquipInventoryDetail(GDB::itemdata * list, unsigned int size);

	unsigned int GetPocketInventorySize();
	int GetPocketInventoryDetail(GDB::pocket_item* list, unsigned int size);
	
	unsigned int GetFashionInventorySize();
	int GetFashionInventoryDetail(GDB::itemdata* list, unsigned int size);

	unsigned int GetMountWingInventorySize();
	int GetMountWingInventoryDetail(GDB::itemdata* list, unsigned int size);

	unsigned int GetGiftInventorySize();
	int GetGiftInventoryDetail(GDB::itemdata* list, unsigned int size);

	unsigned int GetFuwenInventorySize();
	int GetFuwenInventoryDetail(GDB::itemdata* list, unsigned int size);
public:
	//造一个小弟
	struct minor_param
	{
	...
	bool CreatePet(const A3DVECTOR& pos,const pet_data* pData,const pet_bedge_essence* pEss,int tid,unsigned int pet_index,XID& who, int reborn_cnt, void * cd_buf, unsigned int cd_len);
	...
	bool CreateSummon(int id, int count, int lifetime, char type, int skill_level, const XID & target, bool use_target_pos); 
	bool CreateClone(int id, int count, int lifetime, char type, int skill_level, bool exchange); 
	...
	bool CreateTrap(int id, int lifetime, char type, int skill_level, bool is_invisible, int max_count); 
	bool CreateTelePort(int id, int lifetime, char type, int skill_level, bool is_invisible, int skill_id, int skill_cd, int teleport_count, int teleport_prob); 
	bool CreatePosSummon(int id, int count, int lifetime, char type, int skill_level, const XID & target);
	bool SummonMonster(summon_param & param);
	void UnSummonMonster(char type);
	...
	void TryCharge(int type, const A3DVECTOR & dest_pos, int target_id);
	...
	void CurseTeleport(const XID & who);
	void SpyTrap();
	void SetPuppetForm(bool on, int num);
	void SetPuppetSkill(int skill_id, int immune_prob);
	void SetQilinForm(bool on);

	void BounceTo(const A3DVECTOR & target_pos, float dist);
	void DrawTo(const XID & target, const A3DVECTOR & pos);
	void Repel2(const A3DVECTOR & target_pos, float dist);
	void Blowoff();
	void Throwback(const A3DVECTOR & target_pos, float dist);

	void CastCycleSkill(int skill_id, int skill_level, char skill_type, float radius, int coverage, int mpcost, int hpcost, 
			int dpcost, int inkcost, char force, float ratio, int plus, int period, int var[16], int crit_rate); 

	void SetRage(int rage);
	void AddRage(int rage);

	int GetPuppetid();	
	void ReduceSkillCooldown(int id, int cooldown_msec);
	void NotifyBloodPoolStatus(bool on, int cur_hp, int max_hp);
public:
	//阵法相关
	/**
	 ...
	 */
	bool CircleOfDoomPrepare( float radius, int faction, int max_member_num, int skill_id);
	...
	void DeliverTopicSite( TOPIC_SITE::topic_data* data );  // Youshuang add

	const XID GetQilinLeaderID();
	
	void BeHurtOnSeekAndHideRaid(const XID& who);
