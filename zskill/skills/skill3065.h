#ifndef __CPPGEN_GNET_SKILL3065
#define __CPPGEN_GNET_SKILL3065
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill3065:public Skill
    {
      public:
        enum
        { SKILL_ID = 3065 };
          Skill3065 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill3065Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
 ...
 1237600 };
        ...
        int GetDescription (Skill * skill, wchar_t * buffer, int length, wchar_t * format, const SkillStr & table) const
        {
            return _snwprintf (buffer, length, format,
                               ...
                               225 * skill->GetLevel (), 12 + skill->GetLevel (), 8, 5 + 2 * skill->GetLevel ());
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetValue (225 * skill->GetLevel () * (1 + skill->GetT0 ()));
            skill->GetVictim ()->SetSubattack (1);
            skill->GetVictim ()->SetProbability (1.0 * (5 + 2 * skill->GetLevel ()));
            skill->GetVictim ()->SetTime (skill->GetLevel () > 19 ? 4050 : 3050);
