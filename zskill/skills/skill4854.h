#ifndef __CPPGEN_GNET_SKILL4854
#define __CPPGEN_GNET_SKILL4854
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill4854:public Skill
    {
      public:
        enum
        { SKILL_ID = 4854 };
          Skill4854 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill4854Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
          ...
          public:
            ...
            void Calculate (Skill * skill) const
            {
                ...
                int dam =
                  ...
                  skill->SetPlus (120 + 20 * skill->GetLevel () + dam * cul);
                  skill->SetPlus2 (dam * (1 - cul));
                  skill->SetRatio (1 + (skill->GetPlayer ()->GetCultivation () == 1 ? skill->GetPlayer ()->GetSkilllevel (5253) * 0.05 : 0));
                  ...
                  skill->GetPlayer ()->
                    SetVar1 ((((skill->GetPlayer ()->GetCultivation () == 1)
                               && (skill->GetPlayer ()->GetSkilllevel (5253) >
                                   0)) ? ((skill->GetPlayer ()->GetRes1 () + skill->GetPlayer ()->GetSkilllevel (5253) * 8) >
                                          600 ? (skill->GetPlayer ()->GetRes2 () +
                                                 skill->GetPlayer ()->GetSkilllevel (5253) * 8) : 600) : (skill->GetPlayer ()->GetLevel () * 3 +
                                                                                                          10 * skill->GetLevel ())));
                  skill->GetPlayer ()->SetPerform (1);
            }
        };
#endif
      Skill4854Stub ():SkillStub (4854)
        {
            occupation = 109;
            name = L"±ÎÈÕ";
            nativename = "±ÎÈÕ";
            icon = "¼Æ£ºÕÚÌì±ÎÈÕ.dds";
            maxlevel = 10;
            maxlearn = 9;
            type = 1;
            eventflag = 0;
        ...
        {
            return _snwprintf (buffer, length, format, 120 + 20 * skill->GetLevel (), 10 * skill->GetLevel ());
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetProbability (1.0 * (-1));
            skill->GetVictim ()->SetTime (skill->GetT0 () > 0 ? 3000 : 0);
