#ifndef __CPPGEN_GNET_SKILL3571
#define __CPPGEN_GNET_SKILL3571
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill3571:public Skill
    {
      public:
        enum
        { SKILL_ID = 3571 };
          Skill3571 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill3571Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
 ...
 1450600 };
        ...
        int GetDescription (Skill * skill, wchar_t * buffer, int length, wchar_t * format, const SkillStr & table) const
        {
            return _snwprintf (buffer, length, format,
                               ...
                               120, 2 + int (skill->GetLevel () / 4), 5 + 3 * int (skill->GetLevel () / 5), 1 + int (skill->GetLevel () / 5));
        ...
        bool BlessMe (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetTime ((5 + 3 * int (skill->GetLevel () / 5)) *1000);
            skill->GetVictim ()->SetRatio (1);
            skill->GetVictim ()->SetAmount (40);
