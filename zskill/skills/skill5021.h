#ifndef __CPPGEN_GNET_SKILL5021
#define __CPPGEN_GNET_SKILL5021
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill5021:public Skill
    {
      public:
        enum
        { SKILL_ID = 5021 };
          Skill5021 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill5021Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
      ...
      Skill5021Stub ():SkillStub (5021)
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetProbability (1.0 * (skill->GetPlayer ()->GetVar2 () + 10));
            skill->GetVictim ()->SetTime (1000 *
                                          (1 +
                                           (norm (skill->GetLevel () / 30) + norm (skill->GetLevel () / 60) + norm (skill->GetLevel () / 90) +
                                            norm (skill->GetLevel () / 100))));
            skill->GetVictim ()->SetSilent (1);
            skill->GetVictim ()->SetProbability (1.0 * (skill->GetPlayer ()->GetVar3 () + 10));
            skill->GetVictim ()->SetTime (1000 *
                                          (1 +
                                           (norm (skill->GetLevel () / 30) + norm (skill->GetLevel () / 60) + norm (skill->GetLevel () / 90) +
                                            norm (skill->GetLevel () / 100))));
