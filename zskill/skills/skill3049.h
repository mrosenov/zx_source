#ifndef __CPPGEN_GNET_SKILL3049
#define __CPPGEN_GNET_SKILL3049
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill3049:public Skill
    {
      public:
        enum
        { SKILL_ID = 3049 };
          Skill3049 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill3049Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
          ...
          public:
            ...
            void Calculate (Skill * skill) const
            {
                ...
                skill->SetPlus (0.071 * skill->GetLevel () * skill->GetLevel () + 13 * skill->GetLevel () + 1027 +
                                skill->GetPlayer ()->GetRes2 () * (2 + 0.2 * skill->GetLevel ()) * skill->GetJ0 ());
                skill->GetPlayer ()->SetPerform (1);
            }
        };
#endif
      Skill3049Stub ():SkillStub (3049)
        {
            occupation = 167;
            name = L"神引：阳关三叠<玄>";
            nativename = "神引：阳关三叠<玄>";
            icon = "仙・神引：阳关三叠.dds";
            maxlevel = 20;
            maxlearn = 20;
            type = 1;
 ...
 1237600 };
        ...
        int GetDescription (Skill * skill, wchar_t * buffer, int length, wchar_t * format, const SkillStr & table) const
        {
            return _snwprintf (buffer, length, format,
                               ...
                               0.3 * skill->GetLevel (), 0.2 * skill->GetLevel (), 2 + 0.2 * skill->GetLevel (), 20 + 2 * skill->GetLevel ());
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetValue (65);
            skill->GetVictim ()->SetDispel (1);
            skill->GetVictim ()->SetProbability (1.0 * (-1));
            skill->GetVictim ()->SetRatio (74);
