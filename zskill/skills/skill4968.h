...
namespace GNET
{
    ...
    class Skill4968Stub:public SkillStub
    {
      public:
        ...
        {
            return _snwprintf (buffer, length, format, 2 + 0.5 * skill->GetLevel (), 50 * skill->GetLevel (), 5 + 4 * skill->GetLevel ());
        ...
        {
            skill->GetVictim ()->SetProbability (1.0 * (skill->GetPlayer ()->GetVar6 ()));
            skill->GetVictim ()->SetTime (1000 * (2 + 0.5 * skill->GetLevel ()));
            skill->GetVictim ()->SetDizzy (1);
            skill->GetVictim ()->SetTime (1000 * (5 + 4 * skill->GetLevel ()));
