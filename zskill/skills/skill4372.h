...
namespace GNET
{
    ...
    class Skill4372Stub:public SkillStub
    {
      public:
        ...
        {
            return _snwprintf (buffer, length, format, 2 * skill->GetLevel (), 2 + skill->GetLevel (), 4 + skill->GetLevel ());
        ...
        {
            skill->GetVictim ()->SetProbability (1.0 * (zrand (100) < 2 * skill->GetLevel ()? 750 : 0));
            skill->GetVictim ()->SetTime (1000 * (2 + skill->GetLevel ()));
            skill->GetVictim ()->SetSilent (1);
            skill->GetVictim ()->SetProbability (1.0 * (-1));
            skill->GetVictim ()->SetTime (2000);
            skill->GetVictim ()->SetBuffid (1);
            skill->GetVictim ()->SetAmount (skill->GetPlayer ()->GetMaxhp () * 0.01 * (4 + skill->GetLevel ()));
