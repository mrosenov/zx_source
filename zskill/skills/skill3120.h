#ifndef __CPPGEN_GNET_SKILL3120
#define __CPPGEN_GNET_SKILL3120
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill3120:public Skill
    {
      public:
        enum
        { SKILL_ID = 3120 };
          Skill3120 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill3120Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
        ...
        class State7:public SkillStub::State
        {
          public:
            ...
            {
                skill->GetPlayer ()->SetVar1 (skill->GetPlayer ()->GetRes2 ());
                skill->SetRatio (1.12 + 0.05 * skill->GetLevel () + norm (skill->GetLevel () / 14.9) * 0.03 * (skill->GetLevel () - 15) + 0.15);
                skill->SetPlus ((720 + 5 * skill->GetLevel () * skill->GetLevel ()) * (1 + 0.07 * skill->GetT3 ()));
                skill->GetPlayer ()->SetPerform (0);
            }
        };
#endif
      Skill3120Stub ():SkillStub (3120)
        {
            occupation = 169;
            name = L"Î´ÃûÕ¶<ìø>";
            nativename = "Î´ÃûÕ¶<ìø>";
        ...
        int GetDescription (Skill * skill, wchar_t * buffer, int length, wchar_t * format, const SkillStr & table) const
        {
            return _snwprintf (buffer, length, format,
                               ...
                               8, 33, 30, 30, 6, 2 * skill->GetLevel (), 33, 10 * skill->GetLevel (), 15, 1 + 0.1 * skill->GetLevel (), 3);
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetProbability (1.0 * (3 + norm (skill->GetLevel () / 19.9) * 2));
            skill->GetVictim ()->SetTime (1050 + 100 * skill->GetLevel ());
