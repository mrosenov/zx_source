#ifndef __CPPGEN_GNET_SKILL3077
#define __CPPGEN_GNET_SKILL3077
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill3077:public Skill
    {
      public:
        enum
        { SKILL_ID = 3077 };
          Skill3077 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill3077Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
 ...
 1237600 };
        ...
        int GetDescription (Skill * skill, wchar_t * buffer, int length, wchar_t * format, const SkillStr & table) const
        {
            return _snwprintf (buffer, length, format,
                               ...
                               INT (180 + 2 * skill->GetLevel () * skill->GetLevel ()),
                               19 + skill->GetLevel (), 5 * skill->GetLevel (), 3 * skill->GetLevel ());
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetProbability (1.0 * ((19 + skill->GetLevel () + 5 * skill->GetT3 ())));
            skill->GetVictim ()->SetAmount (2);
            skill->GetVictim ()->SetClearbuff (1);
            skill->GetVictim ()->SetProbability (1.0 * (5 * skill->GetLevel ()));
            skill->GetVictim ()->SetTime (6000 + 100);
            skill->GetVictim ()->SetRatio (skill->GetLevel () > 19 ? 0.03 * skill->GetLevel () + 0.15 : 0.03 * skill->GetLevel ());
