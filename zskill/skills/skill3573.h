#ifndef __CPPGEN_GNET_SKILL3573
#define __CPPGEN_GNET_SKILL3573
namespace GNET
{
#ifdef _SKILL_SERVER
    class Skill3573:public Skill
    {
      public:
        enum
        { SKILL_ID = 3573 };
          Skill3573 ():Skill (SKILL_ID)
        {
        }
    };
#endif
    class Skill3573Stub:public SkillStub
    {
      public:
#ifdef _SKILL_SERVER
...
#endif
      Skill3573Stub ():SkillStub (3573)
        {
            occupation = 167;
            name = L"ÈýÉúÅÐ<Ðþ>";
            nativename = "ÈýÉúÅÐ<Ðþ>";
            icon = "ÏÉ¡¤ÈýÉúÅÐ.dds";
            maxlevel = 20;
            maxlearn = 20;
            type = 1;
 ...
 1450600 };
        ...
        int GetDescription (Skill * skill, wchar_t * buffer, int length, wchar_t * format, const SkillStr & table) const
        {
            return _snwprintf (buffer, length, format,
                               ...
                               5 + skill->GetLevel (),
                               skill->GetLevel (),
                               3 * skill->GetLevel (), INT (1 + skill->GetLevel () / 5), skill->GetLevel (), 2 + int (skill->GetLevel () / 5));
        ...
        bool StateAttack (Skill * skill) const
        {
            ...
            skill->GetVictim ()->SetProbability (1.0 *
                                                 (skill->GetPlayer ()->GetVar1 () - skill->GetPlayer ()->GetRes4 () +
                                                  20 * norm (skill->GetLevel () / 19.99)));
            skill->GetVictim ()->SetTime (2000 + INT (skill->GetLevel () / 5) * 1000 + norm (skill->GetLevel () / 19.99) * 5000);
